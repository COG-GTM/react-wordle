...
function App() {
  ...
  const activeSolution = isUnlimitedMode ? unlimitedState.solution : solution;
  const activeGuesses = isUnlimitedMode ? unlimitedState.guesses : guesses;
  const activeStats = isUnlimitedMode ? unlimitedStats : stats;
  const isGameWon = activeGuesses.includes(activeSolution.toUpperCase());
  const isGameLost = !isGameWon && activeGuesses.length === MAX_CHALLENGES;
  ...
  const handleUnlimitedMode = () => {
    if (!isUnlimitedMode && !unlimitedState.solution) {
      setUnlimitedState({
        solution: getRandomWord(solution),
        guesses: [],
      });
    }
  ...
  const handlePlayAgain = () => {
    setUnlimitedState({
      solution: getRandomWord(unlimitedState.solution),
      guesses: [],
    });
    setCurrentGuess('');
  ...
  const handleEnter = () => {
    ...
    const setActiveStats = isUnlimitedMode ? setUnlimitedStats : setStats;

    if (currentGuess === activeSolution.toUpperCase()) {
      setActiveStats(
        addStatsForCompletedGame(activeStats, activeGuesses.length)
      );
      ...
      setActiveStats(
        addStatsForCompletedGame(activeStats, activeGuesses.length + 1)
      );
      setTimeout(
    ...
    if (isUnlimitedMode) {
      setUnlimitedState({
        ...unlimitedState,
        guesses: [...unlimitedState.guesses, currentGuess],
      });
    } else {
  ...
  return (
    <div className={styles.container}>
      ...
      <StatsModal
        ...
        numberOfGuessesMade={activeGuesses.length}
        isGameWon={isGameWon}
