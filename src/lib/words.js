...
export const getGuessStatuses = (guess, activeSolution) => {
  const splitGuess = guess.toLowerCase().split('');
  const splitSolution = activeSolution.toLowerCase().split('');
...
export const getStatuses = (guesses, activeSolution) => {
  const charObj = {};
  const splitSolution = activeSolution.toUpperCase().split('');
...
export const findFirstUnusedReveal = (word, guesses, activeSolution) => {
  if (guesses.length === 0) {
    return false;
  }
  ...
  const statuses = getGuessStatuses(guess, activeSolution);
...
\n` + generateEmojiGrid(guesses, activeSolution);

  navigator.clipboard.writeText(textToShare);
};

export const generateEmojiGrid = (guesses, activeSolution) => {
  return guesses
    .map(guess => {
      const status = getGuessStatuses(guess, activeSolution);
