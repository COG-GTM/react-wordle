...
export const getGuessStatuses = (guess, gameSolution = solution) => {
  const splitGuess = guess.toLowerCase().split('');
  const splitSolution = gameSolution.split('');
...
export const getStatuses = (guesses, gameSolution = solution) => {
  const charObj = {};
  const splitSolution = gameSolution.toUpperCase().split('');
...
export const findFirstUnusedReveal = (
  ...
  const statuses = getGuessStatuses(guess, gameSolution);
...
export const shareStatus = (
  ...
  const textToShare =
    `Wordle Game
${isUnlimitedMode ? '∞' : `#${solutionIndex}`} 
${isGameLost ? 'X' : guesses.length}/${MAX_CHALLENGES} 
${isHardMode ? 'Hard Mode' : ''}
\n` + generateEmojiGrid(guesses, gameSolution);

  navigator.clipboard.writeText(textToShare);
};

export const generateEmojiGrid = (guesses, gameSolution = solution) => {
  return guesses
    .map(guess => {
      const status = getGuessStatuses(guess, gameSolution);
...
export const getRandomWord = (excludeWord = '') => {
  ...
  while (WORDS.length > 1 && word === excludeWord) {
