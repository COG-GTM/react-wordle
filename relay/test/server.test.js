...
test('rejects joining a room that has passed its unjoined TTL', async () => {
  let now = 0;
  const store = new InMemoryRoomStore({
    clock: () => now,
    unjoinedTtlMs: 10,
  ...
  const guest = connect(port);
  await waitOpen(guest);
  const errorMessage = nextMessage(guest);
  guest.send(
    JSON.stringify({
      v: 1,
      type: 'join_room',
  ...
  assert.equal((await errorMessage).payload.code, 'ROOM_EXPIRED');

  await Promise.all([closeSocket(host), closeSocket(guest)]);
  await relay.close();
});
...
test('rejects a missing origin during upgrade', async () => {
  const relay = createRelayServer(config());
  const port = await startServer(relay);
  await assert.rejects(
    () =>
...
test('rate-limits connection attempts from one IP', async () => {
  const relay = createRelayServer(
    config({ JOIN_RATE_LIMIT: 1, JOIN_RATE_WINDOW_MS: 60000 })
  );
  const port = await startServer(relay);
