...
function createRelayServer(
  ...
  const broadcastRoom = async (
    ...
    for (const session of [...sessions]) {
      ...
      if (close) session.ws.close(1000, closeReason);
    }
  };
  ...
  const sweepTimer = setInterval(() => {
    pruneJoinLimiters(joinLimiters);
    void store.sweep().catch(error => {
      log('sweep_failed', { config, error: error.message });
    });
  ...
  async function close() {
    ...
    for (const roomCode of roomCodes) {
      await broadcastRoom(
        ...
        true,
        null,
        CLOSE_REASONS.SHUTDOWN
      );
    }
    for (const session of sessions) {
      session.ws.close(1000, CLOSE_REASONS.SHUTDOWN);
    }
